    ...
    document.addEventListener('keydown', (event) => {
        ...
        switch (event.key) {
            ...
            case 'ArrowRight':
                ...
                if (currentIndex === items.length - 1) { // Jika sudah di item terakhir
                    nextIndex = items.length - 1; // Tetap di item terakhir
                }
                if (currentIndex === -1 && items.length > 0) { // Jika tidak ada item aktif dan ada item
                    nextIndex = 0; // Pilih item pertama
                }

                // Pastikan nextIndex valid
                if (nextIndex !== -1 && nextIndex < items.length) {
                    const currentItem = items[currentIndex]; // Bisa undefined jika currentIndex adalah -1
                    const nextItem = items[nextIndex];

                    // Terapkan logika khusus hanya untuk tab 'display' dan jika ada perpindahan item yang valid
                    if (activeTabId === 'display' && currentItem && nextItem && currentIndex !== nextIndex) {
                        const currentRawText = currentItem.dataset.rawText;
                        const nextRawText = nextItem.dataset.rawText;

                        if (typeof window.handleArrowRightDisplay === 'function') {
                            window.handleArrowRightDisplay(currentRawText, nextRawText, nextItem, container, itemSelector);
                            return; // Keluar dari switch, karena tampilan sudah ditangani
                        }
                    }
                }
        ...
        // Modifikasi: Jangan panggil setActiveItem jika ArrowRight di tab 'display' sudah ditangani oleh window.handleArrowRightDisplay
        const arrowRightHandledByCpJs = event.key === 'ArrowRight' && activeTabId === 'display' && typeof window.handleArrowRightDisplay === 'function';

        if (!arrowRightHandledByCpJs && nextIndex !== -1 && nextIndex < items.length && (nextIndex !== currentIndex || currentIndex === -1)) {
            ...
            // if (event.key === 'ArrowRight' && activeTabId === 'display') {
            //     // Jika ArrowRight di display tab dan sudah ditangani, jangan log "No valid next item"
            // } else
            // console.log('No valid next item to activate or index out of bounds.');
            if (!arrowRightHandledByCpJs) { // Hanya log jika tidak ditangani secara khusus
                console.log('No valid next item to activate or index out of bounds.');
            }
            console.log('No valid next item to activate or index out of bounds.');
        }

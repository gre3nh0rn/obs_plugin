...
// --- MODIFIKASI UNTUK HOTKEY ARROW RIGHT ---
// Fungsi ini akan dipanggil dari hotkey.js
// Pastikan browser.html memiliki CSS yang sesuai untuk kelas .multi-line-display-container, .line-previous, .line-current,
// dan animasi @keyframes cpJsFadeOutAndShrink (CSS ini sudah ada di browser.html Anda).

window.handleArrowRightDisplay = function(currentRawText, nextRawText, nextItemElement, itemsContainer, itemSelector) {
    // Hapus kelas 'active-group' dari item yang aktif saat ini di panel kontrol (jika ada)
    if (itemsContainer) {
        const currentActiveInContainer = itemsContainer.querySelector(`${itemSelector}.active-group`);
        if (currentActiveInContainer) currentActiveInContainer.classList.remove('active-group');
    }
    // Tambahkan kelas 'active-group' ke item berikutnya dan scroll ke tampilan
    nextItemElement.classList.add('active-group');
    nextItemElement.scrollIntoView({ behavior: 'smooth', block: 'nearest' });

    // Buat HTML gabungan untuk ditampilkan di browser.html
    // applyFormatting() akan memastikan teks diformat dengan benar (misalnya, untuk not angka, bold, italic)
    const combinedHtml = `<div class="multi-line-display-container"><div class="line-previous">${applyFormatting(currentRawText || '')}</div><div class="line-current">${applyFormatting(nextRawText || '')}</div></div>`;
    openBrowserWindow(combinedHtml, 'editor_arrow_right_nav'); // 'editor_arrow_right_nav' adalah source baru untuk identifikasi
};
